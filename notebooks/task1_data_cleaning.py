...
null_report = pd.DataFrame({"null_count": null_count,"null_percent": null_percent})
null_report = null_report.sort_values("null_percent",ascending=False)
print("\nTop 10 Most Missing Columns:")
print(null_report.head(10))
# DROP HIGHLY MISSING COLUMNS
df = df.dropna(axis=1,thresh=len(df) * 0.60)
...
if "loan_status" in df.columns:
    df = df[~df["loan_status"].astype(str).str.contains("Does not meet the credit policy", na=False)]
# FEATURE ENGINEERING
# 1. NPA FLAG
df["is_npa"] = df["loan_status"].isin(["Charged Off","Default"]).astype(int)
# 2. ISSUE YEAR
...
if ("issue_d" in df.columns and "last_pymnt_d" in df.columns):
    ...
    df["loan_age_months"] = ( (df["last_pymnt_d"] - df["issue_d"]).dt.days) / 30
# GRADE NUMERIC
grade_map = {
    "A": 1,
    "B": 2,
